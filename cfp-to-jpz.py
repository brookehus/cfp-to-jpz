        ...
        with open(self.cfp_filename, 'r') as cfp_file:
            for i, row in enumerate(cfp_file):
                if row[1:6] == 'TITLE':
                    ...
                    copyright = row.lstrip('<COPYRIGHT>').rstrip('</COPYRIGHT>\n')
...
class Jpz():
    ...
    def encode_metadata(self):
        ...
        metadata_strings.append(
            '<?xml version="1.0" encoding="UTF-8"?>{}{}'.format(self.lb, self.lb)
        )
    ...
    def encode_locations(self):
        ...
        for a in range(1, np.max(self._xw._numbering)+1):
            if a in cat_across_starts:
                location_strings.append('<word id="{}0000">{}'.format(a, self.lb))
            ...
            if a in cat_down_starts:
                location_strings.append('<word id="{}0001">{}'.format(a, self.lb))
    ...
    def encode_clues(self):
        ...
        for clue_num, clue in self._xw.clue_dict['across'].items():
            clue_strings.append(
            '<clue number="{}" word="{}0000">{}'.format(clue_num, clue_num, self.lb)
            )
            clue_strings.append(
            '{}<span>{}</span>{}</clue>{}'.format(self.tab, clue, self.lb, self.lb)
            )
        ...
        for clue_num, clue in self._xw.clue_dict['down'].items():
            clue_strings.append(
            '<clue number="{}" word="{}0001">{}'.format(clue_num, clue_num, self.lb)
            )
            clue_strings.append(
            '{}<span>{}</span>{}</clue>{}'.format(self.tab, clue, self.lb, self.lb)
            )
